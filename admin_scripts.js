...
function checkServerStatus() {
  ...
  ["server1", "server2"].forEach((server) => {
    setTimeout(() => {
      ...
      // 전역 변수에 저장된 서버 상태를 기반으로 상태 및 ping 표시
      if (serverStatus[server] === "일시 정지") {
        statusElement.innerText = "일시 정지됨";
        statusElement.classList.remove("active-light");
        statusElement.classList.add("paused-light"); // 붉은 불 표시
        pauseButton.disabled = true;
        pingElement.innerText = ""; // 핑 정보 없음
      } else {
        statusElement.innerText = "활성화";
        statusElement.classList.remove("paused-light");
        statusElement.classList.add("active-light"); // 초록 불 표시
        pauseButton.disabled = false;

        // 활성화 상태일 때 핑을 10~13ms 사이로 랜덤 반환
...
function pauseServer(server) {
  setTimeout(() => {
    ...
    // 핑 정보와 버튼 상태 업데이트
    document.getElementById(`${server}-ping`).innerText = ""; // 핑 정보 없음
    document.getElementById(`pause-${server}`).disabled = true;

    // 2초 후 서버 상태 확인 수행
    setTimeout(checkServerStatus, 2000); // 2초 후 checkServerStatus 호출
  }, 2000); // 일시 정지 시간을 2초로 설정
}
...
function startServer(server) {
  ...
  let steps = [
    ...
    { text: "웹서비스 재시작중입니다. 약 20초 소요됩니다.", time: 12000, width: 20 },
    { text: "웹서비스 시작됨. 서버: service.biilab.kro.kr, 포트:5555", time: 3000, width: 25 },
    ...
    { text: "서비스 활성화됨: service.biilab.kro.kr:5555", time: 0, width: 100 }
  ];
